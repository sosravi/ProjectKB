...
resource "aws_api_gateway_resource" "content_confirm" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  parent_id   = aws_api_gateway_resource.content.id
...
# API Gateway Method for confirm upload
resource "aws_api_gateway_method" "content_confirm_post" {
  rest_api_id   = aws_api_gateway_rest_api.main.id
  resource_id   = aws_api_gateway_resource.content_confirm.id
  http_method   = "POST"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "content_confirm" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  resource_id = aws_api_gateway_resource.content_confirm.id
  http_method = aws_api_gateway_method.content_confirm_post.http_method
  type        = "AWS_PROXY"
  uri         = "arn:aws:apigateway:${var.aws_region}:lambda:path/2015-03-31/functions/arn:aws:lambda:${var.aws_region}:${data.aws_caller_identity.current.account_id}:function:${var.project_name}-${var.environment}-content-confirm-upload/invocations"
  
  integration_http_method = "POST"
}

# Lambda permission for API Gateway to invoke content-confirm-upload
resource "aws_lambda_permission" "api_gateway_content_confirm" {
